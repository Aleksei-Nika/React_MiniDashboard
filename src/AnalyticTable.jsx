...
function AnalyticTable({data}){
    const tableContainerRef = useRef(null);
    const [showScrollTop, setShowScrollTop] = useState(false);

    //храним ширину колонки, но обновляем только в момент mouseup
    const [colWidth, setColWidth] = useState({title: 400, amount: 150, category: 150, status: 150});
    ...
    // 2/ Логика Resize через useRef напрямую в DOM
    ...
    const handleMouseUp = () => {
        ...
        if (info.isResizing && info.currentColumn){
            //  только ТЕПЕРЬ фиксируем в стейт
                ...
                <table style={{tableLayout:'fixed', width:'100%', borderCollapse:'collapse'}}>
                    <thead>
                        <tr style={{position:'sticky', top:0, zIndex:1, backgroundColor:'#f0f0f0'}}>
                            ...
                            <th style={{width: colWidth.status, position:'relative', border: '1px solid #ddd'}}>
                                Дата                       
                            </th>
                        </tr>
                    </thead>
